<?php
// Autoloader
$autoloader = new \App\System\Engine\Autoloader();
$autoloader->register('App\\' . APPLICATION, DIR_APPLICATION);
$autoloader->register('App\Extension', DIR_EXTENSION);
$autoloader->register('App\System', DIR_SYSTEM);
...
// Registry
$registry = new \App\System\Engine\Registry();
$registry->set('autoloader', $autoloader);

// Config
$config = new \App\System\Engine\Config();
...
// Logging
$log = new \App\System\Library\Log($config->get('error_filename'));
...
// Event
$event = new \App\System\Engine\Event($registry);
...
if ($config->has('action_event')) {
	foreach ($config->get('action_event') as $key => $value) {
		foreach ($value as $priority => $action) {
			$event->register($key, new \App\System\Engine\Action($action), $priority);
		}
	}
}

// Factory
$registry->set('factory', new \App\System\Engine\Factory($registry));

// Loader
$loader = new \App\System\Engine\Loader($registry);
$registry->set('load', $loader);

// Request
$request = new \App\System\Library\Request();
...
// Response
$response = new \App\System\Library\Response();
...
if ($config->get('db_autostart')) {
	$db = new \App\System\Library\DB($config->get('db_engine'), $config->get('db_hostname'), $config->get('db_username'), $config->get('db_password'), $config->get('db_database'), $config->get('db_port'), $config->get('db_ssl_key'), $config->get('db_ssl_cert'), $config->get('db_ssl_ca'));
	$registry->set('db', $db);
}

// Session
if ($config->get('session_autostart')) {
	$session = new \App\System\Library\Session($config->get('session_engine'), $registry);
...
// Cache
$registry->set('cache', new \App\System\Library\Cache($config->get('cache_engine'), $config->get('cache_expire')));

// Template
$template = new \App\System\Library\Template($config->get('template_engine'));
$template->addPath(DIR_TEMPLATE);
$registry->set('template', $template);

// Language
$language = new \App\System\Library\Language($config->get('language_code'));
$language->addPath(DIR_LANGUAGE);
$language->load('default');
$registry->set('language', $language);

// Url
$registry->set('url', new \App\System\Library\Url($config->get('site_url')));

// Document
$registry->set('document', new \App\System\Library\Document());

$action = '';
$args = [];

// Action error object to execute if any other actions cannot be executed.
$error = new \App\System\Engine\Action($config->get('action_error'));

// Pre Actions
foreach ($config->get('action_pre_action') as $pre_action) {
	$pre_action = new \App\System\Engine\Action($pre_action);

	$result = $pre_action->execute($registry, $args);

	if ($result instanceof \App\System\Engine\Action) {
		$action = $result;
...
if (str_contains($route, '._')) {
	$action = new \App\System\Engine\Action($config->get('action_error'));
}
...
// Action to execute
if (!$action) {
	$action = new \App\System\Engine\Action($route);
}
...
while ($action) {
	...
	if ($output instanceof \App\System\Engine\Action) {
		$action = $output;
	}
